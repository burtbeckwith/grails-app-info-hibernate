class SimpleTest extends AbstractAppInfoWebTest {
	...
	void testHibernate() {
		...
		for (string in ['org.hibernate.dialect.H2Dialect', 'com.burtbeckwith.appinfo_test.UserRole',
		                'DROP TABLE IF EXISTS database_update', 'gonzoBooks', 'maxBookTitleLength',
		                'com.burtbeckwith.appinfo_test.MyUserType']) {
			assert content.contains(string)
		}

		assert content.contains('org.hibernate.cache.EhCacheProvider') || content.contains('net.sf.ehcache.hibernate.EhCacheRegionFactory')
	}
	...
	void testHibernateHbm() {
		...
		assertContentContains '<class name="com.burtbeckwith.appinfo_test.Role" table="role" mutable="false"'
		assertContentContains 'persister="org.codehaus.groovy.grails.orm.hibernate.persister.entity.GroovyAwareSingleTableEntityPersister">'
	}
}
