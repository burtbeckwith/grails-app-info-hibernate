...
target(createAppInfoHibernateTestApps: 'Creates test apps for functional tests') {
...
private void init(String name, config) {
	...
	def pluginZip = new File(basedir, "grails-app-info-hibernate-${pluginVersion}.zip")
	if (!pluginZip.exists()) {
	...
	appName = 'appinfo-hibernate-test-' + name
...
private void createApp() {
	...
	File ds = new File(testprojectRoot, 'grails-app/conf/DataSource.groovy')
	String fixed = ds.text.replace('cache.use_query_cache = false', 'cache.use_query_cache = true')
	ds.withWriter { it.writeLine fixed }
}
...
runtime ":app-info-hibernate:$pluginVersion"
""")
...
grails.plugins.dynamicController.mixins = [
	...
	'com.burtbeckwith.grails.plugins.appinfo.hibernate.HibernateControllerMixin':   'com.burtbeckwith.appinfo_test.AdminManageController'
]
...
setDefaultTarget 'createAppInfoHibernateTestApps'
